...
public:
    ...
    {
        LogMessage(spdlog::level::trace, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }

    template <typename... Args>
    static void Info(const char *msg, Args &&...args)
    {
        LogMessage(spdlog::level::info, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }

    template <typename... Args>
    static void Debug(const char *msg, Args &&...args)
    {
        LogMessage(spdlog::level::debug, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }

    template <typename... Args>
    static void Warn(const char *msg, Args &&...args)
    {
        LogMessage(spdlog::level::warn, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }

    template <typename... Args>
    static void Error(const char *msg, Args &&...args)
    {
        LogMessage(spdlog::level::err, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }

    template <typename... Args>
    static void Critical(const char *msg, Args &&...args)
    {
        LogMessage(spdlog::level::critical, fmt::format(fmt::runtime(msg), std::forward<Args>(args)...));
    }
